...
void initialize_world_from_file(const char * filename) {
	...
	int MAXWIDTH = 256;
	int cur_line = 0;
	char next_line[WORLDHEIGHT][MAXWIDTH];
	FILE* input_world = fopen(filename, "r");
	if(input_world==NULL){
		fprintf(stderr,"File does not exist or cannot be opened.\n");
		abort();
	}
	while(fgets(next_line[cur_line], MAXWIDTH, input_world)!=NULL && cur_line<WORLDHEIGHT) {
		cur_line++;
	}
	int i, j;
	...
	for(x=0;x<WORLDWIDTH;x++){
		for(y=0;y<WORLDHEIGHT;y++){
			if(y<cur_line&&next_line[y][x]==CHAR_ALIVE){
				world[x][y]=ALIVE;
			} else {
				world[x][y]=DEAD;
			}	
...
void save_world_to_file(const char * filename) {
	 ...
	 */
	FILE* output_world = fopen(filename, "w");
	int x = 0, y = 0;
	char w_output[WORLDWIDTH+2];
	
	for(y=0;y<WORLDHEIGHT;y++){
		for(x=0;x<WORLDWIDTH;x++) {	
			w_output[x]=world[x][y]==ALIVE?CHAR_ALIVE:CHAR_DEAD;	
		}
		w_output[WORLDWIDTH]='\n';
		w_output[WORLDWIDTH+1]='\0';
		fputs(w_output,output_world);
	}
